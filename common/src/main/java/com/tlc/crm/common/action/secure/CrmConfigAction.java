...
public abstract class CrmConfigAction<T extends TlcModel> extends CrmSecureAction {
    ...
    public final JsonObject sProcess(CrmRequest request) {
        ...
        switch (type) {
            case "create" -> {
                final Collection<T> model = constructFromArray(data);
            ...
            case "update" -> {
                ...
                update(model);
                responseJson.put("updated", model.id());
            }
            case "delete" -> {
                ...
                delete(model);
                responseJson.put("deleted", model.id());
            }
            case "exists" -> {
                final T model = construct(data);
            ...
            case "select" -> {
                ...
                responseJson.put("id " + model.id(), construct(model));
            }
            default -> {
            }
        }
    ...
    private JsonArray getInputs(JsonObject data) {
    ...
    public abstract Collection<T> constructFromArray(JsonObject jsonObject);
